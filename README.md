...
graph LR
    ...
    Backend -->|@register_graph| ScriptWriter[youtube-script-writer]
...
`docker compose up --build -d`만 실행하면 다음이 자동으로 설정됩니다.
- 백엔드가 `http://localhost:8000`에서 시작됩니다.
- OpenWebUI가 `http://localhost:3000`에서 시작됩니다.
- OpenWebUI가 백엔드의 `/v1` OpenAI 호환 엔드포인트를 자동으로 바라봅니다.
- OpenWebUI는 백엔드가 healthy 상태가 된 뒤에 시작됩니다.
- OpenWebUI는 커스텀 이미지로 빌드되어 Tool Calling 패널의 `Input` JSON도 `Output`처럼 pretty print 됩니다.
...
│   └── script_writer/   # youtube-script-writer 그래프
├── tools/
│   └── __init__.py
